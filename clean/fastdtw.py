...
def fastdtw(x, y, radius=1, dist=lambda a, b: abs(a - b)):
    ...
    D, distance, path = fastdtw(x_shrinked, y_shrinked, radius=radius, dist=dist)
...
def dtw(x, y, window=None, dist=lambda a, b: abs(a - b)):
    ...
    for i, j in window:
        dt = dist(x[i-1], y[j-1])
        D[i, j] = min((D[i-1, j][0]+dt, i-1, j, dt), (D[i, j-1][0]+dt, i, j-1, dt), (D[i-1, j-1][0]+dt, i-1, j-1, dt), key=lambda a: a[0])
    path = []
    i, j = len_x, len_y
    while not (i == j == 0):
        path.append((i-1, j-1))
...
def __expand_window(path, len_x, len_y, radius):
    path_ = set(path)
    radius = int(max(len_x,len_y)*radius)
    for i, j in path:
